...
RegisterNetEvent('nafe-YachtHeist:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('nafe-YachtHeist:CustomNotify',message, type)
...
CreateThread(function()
    while true do
        ...
        if Dist <= 50 and not PedSpawned then
            TriggerEvent('nafe-YachtHeist:SpawnNPC',Config.StartPed,Config.StartModel)
            PedSpawned = true
        ...
        if Dist <= 50 then
            ...
            if Dist <= 3 then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('nafe-YachtHeist:RobberyCheck')
                end
            end
        end
        Wait(Sleep)
...
RegisterNetEvent('nafe-YachtHeist:SpawnNPC',function(coords,model)
    ...
    if Config.UseThirdEye then
        if Config.ThirdEyeName == 'ox_target' then
            local options = {
                {
                    name = 'YachtNPC',
                    event = 'nafe-YachtHeist:RobberyCheck',
                    icon = 'fas fa-ship',
...
RegisterNetEvent('nafe-YachtHeist:RobberyCheck', function()
    if not GlobalJob then
        if Config.RequireMinimumLEO then
            local StartRobbery = false
            if Config.UseESX then
                ESX.TriggerServerCallback('nafe-YachtHeist:PoliceAvailable:ESX', function(cb)
                    StartRobbery = cb
                end)                                    
            elseif Config.UseQBCore then
                QBCore.Functions.TriggerCallback('nafe-YachtHeist:PoliceAvailable:QBCore', function(cb)
                    StartRobbery = cb
                end)
            end
            Wait(1000)
            ...
            if StartRobbery then
                GlobalJob = true
                TriggerEvent('nafe-YachtHeist:Client:HeistStart')
                TriggerEvent('nafe-YachtHeist:Notify', Config.Lang['startHeist'], Config.LangType['info'])
            else
                TriggerEvent('nafe-YachtHeist:Notify', Config.Lang['mincops'], Config.LangType['error'])
            end
        else
            GlobalJob = true
            TriggerEvent('nafe-YachtHeist:Client:HeistStart')
        ...
        TriggerEvent('nafe-YachtHeist:Notify', Config.Lang['working'], Config.LangType['error'])
    end
end)

RegisterNetEvent('nafe-YachtHeist:GlobalJobSync', function()
    E1 = true
    E2 = true
    E3 = true
    E4 = true
    GlobalJob = true    
    while GlobalJob do
        TriggerServerEvent('nafe-YachtHeist:Server:GlobalJobSync', true)
        if not GlobalJob then
            break
        end
        Wait(3000)
    end
    TriggerServerEvent('nafe-YachtHeist:Server:GlobalJobSync', false)
end)

RegisterNetEvent('nafe-YachtHeist:Client:GlobalJobSync', function(status)
    if status then
        GlobalJob = true
    else
        GlobalJob = false
...
RegisterNetEvent('nafe-YachtHeist:Client:HeistStart', function()
    if GlobalJob then
        TriggerServerEvent('nafe-YachtHeist:Server:Counter')
        TriggerEvent('nafe-YachtHeist:GlobalJobSync')
    end
end)



RegisterNetEvent('nafe-YachtHeist:Client:EngineLocations', function()
    if Config.UseThirdEye then
                    ...
                    options = {
                        {
                            ...
                            event = 'nafe-YachtHeist:DisableEngine',
                            icon = 'fas fa-engine',
    ...
    elseif Config.Use3DText then
        while true do 
            ...
            if nearEngine then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('nafe-YachtHeist:DisableEngine', data)
                end
            end
            Wait(Sleep)
        end
...
RegisterNetEvent('nafe-YachtHeist:DisableEngine', function(data)
    if not PAlert then
        TriggerEvent('nafe-YachtHeist:PoliceAlert',GetEntityCoords(PlayerPedId()))
        PAlert = true
    end
    ...
    exports['ps-ui']:Maze(function(success)
        if success then
            ...
            TriggerServerEvent('nafe-YachtHeist:Server:EngineSync',data.name, Destroyed)
	    if Destroyed == 4 then
                TriggerEvent('nafe-YachtHeist:Notify', Config.Lang['activeConsole'], Config.LangType['info'])
            end
        else
            TriggerServerEvent('nafe-YachtHeist:server:EngineWine', GetEntityCoords(PlayerPedId()))
        end
    end, time)
end)

RegisterNetEvent('nafe-YachtHeist:client:EngineWine', function (pos)
...
RegisterNetEvent('nafe-YachtHeist:Client:EngineSync', function(name, disabled)
    ...
    TriggerEvent('nafe-YachtHeist:Console')
end)

RegisterNetEvent('nafe-YachtHeist:Console', function()
                    ...
                    options = {
                        {
                            ...
                            event = 'nafe-YachtHeist:ReleaseTrolly',
            ...
            if Dist <= 25 then
                ...
                if Dist <= 3 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        TriggerEvent('nafe-YachtHeist:ReleaseTrolly', data)
                        break
                    end
                end
            end
...
RegisterNetEvent('nafe-YachtHeist:ReleaseTrolly', function(data)
    if data.temp ~= 'trap' then
        TriggerServerEvent('nafe-YachtHeist:ThatIsAThing')
    else
        if not TrollyUp then
            TriggerEvent('nafe-YachtHeist:Notify', Config.Lang['ReleaseedTrolly'], Config.LangType['info'])
            TrollyUp = true
            for k,v in pairs(trolleys) do
                local loc = v.coords
                local TrolleyLoot = math.random(1,100)
                if TrolleyLoot <= Config.GoldBarChance then 
                    TriggerServerEvent('nafe-YachtHeist:Server:TrolleySync', loc, k, 2007413986)  -- Gold Bars
                else 
                    TriggerServerEvent('nafe-YachtHeist:Server:TrolleySync', loc, k, 269934519)  -- Cash
                end
            end
        end
        TriggerEvent('nafe-YachtHeist:Notify', Config.Lang['allDown'], Config.LangType['error'])
    end
end)

RegisterNetEvent('nafe-YachtHeist:Client:TrolleySync', function(loc, k, model)
    if Config.UseThirdEye then
        if Config.ThirdEyeName == 'ox_target' then
            ...
            if Config.ThirdEyeName == 'ox_target' then
                oxTargets[nameSpot] = exports.ox_target:addBoxZone({
                    ...
                    options = {
                        {
                            name = nameSpot,
                            event = 'nafe-YachtHeist:BonusLoot',
                            icon = 'fas fa-hand',
...
RegisterNetEvent('nafe-YachtHeist:Client:ThirdEyeSync', function(data)
    if data then
        if Config.ThirdEyeName == 'ox_target' then
...
RegisterNetEvent('nafe-YachtHeist:BonusLoot', function(data)
    if data.amount == 'trapcard' then
        TriggerServerEvent('nafe-YachtHeist:Server:ThirdEyeSync', data.temp)
        if data.temp == "YachtRareSpot" then
            exports['ps-ui']:VarHack(function(success)
                if success then
                    LootAnim()
                    Wait(5500)
                    TriggerServerEvent('nafe-YachtHeist:Server:BonusLoot', "YachtRareSpot")
                end
             end, 2, 3) -- Number of Blocks, Time (seconds)
        else
            LootAnim()
            Wait(5500)
            TriggerServerEvent('nafe-YachtHeist:Server:BonusLoot', 'Bonus')
        end
    else
        TriggerServerEvent('nafe-YachtHeist:ThatIsAThing')
    end
        ...
        if Config.ThirdEyeName == 'ox_target' then
            oxTargets['Trolley'..k] = exports.ox_target:addBoxZone({
                ...
                options = {
                    {
                        name = 'Trolley'..k,
                        event = 'nafe-YachtHeist:LootTrolly',
                ...
                if TDist < 5 then
                    ...
                    if TDist < 2 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            TriggerEvent("nafe-YachtHeist:LootTrolly")
                        end
                    end
                end
            end
...
RegisterNetEvent('nafe-YachtHeist:Client:LootSync', function(k)
    if k then 
        if Config.UseThirdEye then
            if Config.ThirdEyeName == 'ox_target' then
...
RegisterNetEvent('nafe-YachtHeist:LootTrolly', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    for k,v in pairs(trolleys) do
        if not v.grabbed then
            ...
            if TrolleyDist <= 2 then
                ...
                TriggerServerEvent('nafe-YachtHeist:Server:LootSync', k, true)
                ...
                Wait(1750)
                TriggerEvent('nafe-YachtHeist:GrabTrolley', grabModel)
                ...
                DeleteObject(bag)
                TriggerServerEvent('nafe-YachtHeist:Server:TrolleyReward', grabModel, pos, coords, type)
...
RegisterNetEvent('nafe-YachtHeist:GrabTrolley', function(grabModel)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('nafe-YachtHeist:Reset', function(tolerance)
